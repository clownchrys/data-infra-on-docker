...
dag = DAG(
    dag_id="check_long_duration",
    description="long-lasting task",
...
def func(duration: int, *args, **kwargs):
    print(f"task sleeping for {duration} seconds...")
    time.sleep(duration)
    print("task has been awake!")


generate_task = partial(PythonOperator, python_callable=func, op_kwargs={"duration": 600})

start = DummyOperator(task_id="start", dag=dag)
end = DummyOperator(task_id="end")

sleep_1 = generate_task(task_id="sleep_1")
sleep_2 = generate_task(task_id="sleep_2")

graph = start >> sleep_1 >> sleep_2 >> end
